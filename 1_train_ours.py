...
def train(dataset_name, conf):
    ...
    if conf["TEM"]:
        conf["path_initialization"] = True
#        conf["tree_attention"] = True
#        conf["tree_pooling"] = "average"
        conf["mm_fusion"] = False
        print("TEM testing mode")
...
#    pause()
    ...
    model = ExpMatch(dataset.img_features, len(dataset.meta_code_map), conf["num_hidden"], conf["metric"], conf["loss"], conf["tree_pooling"],conf["tree_attention"], conf["path_encoding"], conf["path_initialization"], conf["meta_interact_attention"], conf["regularization_para"], conf["mm_fusion"],conf["TEM"], dataset.all_paths, dataset.all_masks, dataset.all_Leafnode_weight_mask, dataset.item_featureCode_map, dataset.item_featureMask_map)
    model.cuda()

    lr = conf["lr"]
    optimizer = torch.optim.SGD([
        {'params': model.imageW.parameters(), 'lr': lr},
        {'params': model.item_Fea.parameters(), 'lr': lr},
 #       {'params': model.MLP.parameters(), 'lr': lr},
        {'params': model.W_att.parameters(), 'lr': lr},
        {'params': model.h_att_TEM.parameters(), 'lr': lr},
        {'params': model.h_att.parameters(), 'lr': lr},
#        {'params': model.h_meta_interact_att.parameters(),'lr': lr, 'weight_decay': 1e-4},
        {'params': model.predict1.parameters(), 'lr': lr},
        {'params': model.predict2.parameters(), 'lr': lr},
        {'params': model.meta_embed.parameters(), 'lr': lr, 'weight_decay': 1e-5},
        {'params': model.path_embed_intialized.parameters(), 'lr': lr},
    ...
    for epoch in range(conf["epochs"]):
        for batch_cnt, batch in enumerate(dataset.train_loader): 
            ...
            qry_id, pos_id, neg_id, pos_path, pos_mask, pos_pathids, neg_path, neg_mask, neg_pathids, pos_outfit_ids, neg_outfit_ids = batch
            ...
            pos_pathids = Variable(pos_pathids).cuda()
            neg_path = Variable(neg_path).cuda()
            neg_mask = Variable(neg_mask).cuda()
            neg_pathids = Variable(neg_pathids).cuda()
...
#            print(batchsize)
            ...
            #    pair_loss, outfit_loss = model(qry_id, pos_id, neg_id, pos_path, pos_mask, pos_pathids, neg_path, neg_mask, neg_pathids,  pos_outfit_ids, neg_outfit_ids)
            ...
            #else:
            pair_loss = model(qry_id, pos_id, neg_id, pos_path, pos_mask, pos_pathids, neg_path, neg_mask, neg_pathids, None, None)   
            pair_loss /= float(batchsize)
            log_value("pair_loss", pair_loss, step)
...
def test(model, dataset, conf):
    ...
    pair_score = {}
    if not conf["meta_interact_attention"]:
        all_path_embed = model.generate_all_path_embed()
    else:
        print("Use meta_2_order_interaction_attention")
    ...
    for batch_cnt, batch in enumerate(dataset.test_loader):
        ...
        if conf["meta_interact_attention"]:
            pair, qry_id, res_id,  path, mask, path_ids = batch
            path = Variable(path).cuda()
 ...
 #       print("Round start time:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        ...
        if conf["meta_interact_attention"]:
            score = model.compute_pairwise_score(qry_id, res_id,  path, mask, path_ids, None)#, all_path_embed)
        else:
            score = model.compute_pairwise_score(qry_id, res_id, path_ids, all_path_embed, None, None)  
